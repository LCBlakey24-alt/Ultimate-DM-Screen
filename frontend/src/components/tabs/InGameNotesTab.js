  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h2 className="medieval-heading" style={{ fontSize: '28px', color: '#d4af37', marginBottom: '8px' }}>In-Game Notes</h2>
          <p style={{ fontSize: '14px', color: '#8b7355' }}>Take notes during your session and let AI organize them automatically</p>
        </div>
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          ...
          <DialogContent className="modal" style={{ maxWidth: '700px' }}>
            <DialogHeader>
              ...
              <DialogDescription style={{ color: '#8b7355', marginTop: '8px' }}>
      ...
      {notes.length === 0 ? (
        <Card className="parchment-dark" style={{ padding: '40px', textAlign: 'center' }}>
          <p style={{ color: '#8b7355' }}>No session notes yet. Start taking notes during your game!</p>
        </Card>
      ) : (
      ...
      <Dialog open={showSuggestionsDialog} onOpenChange={setShowSuggestionsDialog}>
        <DialogContent className="modal" style={{ maxWidth: '800px', maxHeight: '80vh', overflow: 'auto' }}>
          <DialogHeader>
            <DialogTitle className="medieval-heading" style={{ fontSize: '24px', color: '#d4af37', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <Sparkles size={28} />
              AI Suggestions
            </DialogTitle>
            <DialogDescription style={{ color: '#8b7355', marginTop: '8px' }}>
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  {aiSuggestions.new_npcs.map((npc, idx) => (
                    <Card key={idx} className="parchment-dark" style={{ marginBottom: '12px', padding: '16px' }}>
                      <h4 style={{ color: '#d4af37', fontSize: '16px', marginBottom: '8px' }}>{npc.name}</h4>
                      <p style={{ color: '#e8dcc4', fontSize: '14px', marginBottom: '12px' }}>{npc.description}</p>
                      {npc.notes && <p style={{ color: '#8b7355', fontSize: '12px', fontStyle: 'italic', marginBottom: '12px' }}>{npc.notes}</p>}
                      <Button
              ...
              {aiSuggestions.new_locations && aiSuggestions.new_locations.length > 0 && (
                <div style={{ marginBottom: '24px' }}>
                  ...
                  {aiSuggestions.new_locations.map((location, idx) => (
                    <Card key={idx} className="parchment-dark" style={{ marginBottom: '12px', padding: '16px' }}>
                      <h4 style={{ color: '#d4af37', fontSize: '16px', marginBottom: '8px' }}>
                        {location.name} {location.type && `(${location.type})`}
                      </h4>
                      <p style={{ color: '#e8dcc4', fontSize: '14px', marginBottom: '12px' }}>{location.description}</p>
                      {location.notes && <p style={{ color: '#8b7355', fontSize: '12px', fontStyle: 'italic', marginBottom: '12px' }}>{location.notes}</p>}
                      <Button
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  {aiSuggestions.new_gods.map((god, idx) => (
                    <Card key={idx} className="parchment-dark" style={{ marginBottom: '12px', padding: '16px' }}>
                      ...
                      <p style={{ color: '#e8dcc4', fontSize: '14px', marginBottom: '12px' }}>{god.description}</p>
                      <Button
              ...
              {((aiSuggestions.npc_updates && aiSuggestions.npc_updates.length > 0) ||
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <Card className="parchment-dark" style={{ padding: '16px' }}>
                    <p style={{ color: '#e8dcc4', fontSize: '14px', lineHeight: '1.6' }}>
                      AI detected updates to existing entities. Please review and manually update:
                    </p>
                    {aiSuggestions.npc_updates?.map((update, idx) => (
                      <div key={idx} style={{ marginTop: '12px', padding: '12px', background: 'rgba(212, 175, 55, 0.1)', borderRadius: '6px' }}>
                        <p style={{ color: '#d4af37', fontSize: '14px', fontWeight: '600' }}>NPC: {update.name}</p>
                        <p style={{ color: '#e8dcc4', fontSize: '13px', marginTop: '4px' }}>{update.additional_notes}</p>
                      </div>
                    ))}
                    {aiSuggestions.location_updates?.map((update, idx) => (
                      <div key={idx} style={{ marginTop: '12px', padding: '12px', background: 'rgba(212, 175, 55, 0.1)', borderRadius: '6px' }}>
                        <p style={{ color: '#d4af37', fontSize: '14px', fontWeight: '600' }}>Location: {update.name}</p>
                        <p style={{ color: '#e8dcc4', fontSize: '13px', marginTop: '4px' }}>{update.additional_notes}</p>
                      </div>
                    ))}
                  </Card>
              ...
              {(!aiSuggestions.new_npcs || aiSuggestions.new_npcs.length === 0) &&
               ...
               (!aiSuggestions.location_updates || aiSuggestions.location_updates.length === 0) && (
                <Card className="parchment-dark" style={{ padding: '32px', textAlign: 'center' }}>
                  <p style={{ color: '#8b7355' }}>No new entities detected in this note.</p>
                </Card>
              )}

              <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '1px solid #5a4a2f' }}>
                <Button
